...
%     drawnow;
% end
function driverMATLAB
% DRIVERMATLAB Run MATLAB-based Barnes-Hut simulation
global simArgs videoWriter

% Use preset based on selection
...
for t = 1:steps
    ...
    for i = 2:size(particles,1)
        body = particles(i,[1,2,5,6]);
        F = root.computeForceOn(body, theta, G, eps);
        a = F / body(3);
        particles(i,3:4) = particles(i,3:4) + a * dt;
        particles(i,1:2) = particles(i,1:2) + particles(i,3:4) * dt;
    end

    scatter(particles(:,1), particles(:,2), 10, 'r', 'filled');
    if simArgs.ViewBoxes
        drawTree(root);
    end
